requirejs(["config"], function(config){
	requirejs(["jquery", "base", "handlebars", "c3"], function($, base, handlebars, c3){
		...
		// Set up WebSocket
		var ws = new WebSocket("ws://" + base.socket_domain + "/stats-socket")
		var numUpdates = 0;
		...
		ws.onmessage = function(e) {
			...
			if (response.hasOwnProperty("stats")) {
				...
				var statValues = $.parseJSON(response.stats);
			...
			if (!chart) {
				...
				//            value: d3.format(',') // apply this format to both y and y2
			    	...
			    	"onresized": function() {
			    		...
			    		// Check screen width, shrink table as necessary
			    		// Media query sets th:first-child overflow to hidden. shrink the site names
			    		if ($("th:first-child").css("overflow") == "hidden"){
	    		// 			$("th:first-child, td:first-child").hover(
							// 	function handlerIn(){
							// 		$("td.site, th.site").css( { width: "15em" } );
							// 	},
							// 	function handlerOut(){
							// 		$("td.site, th.site").css( { width: "5em" } ) ;
							// 	}
							// );
			    		}
			    	},
